...
class SwitchAlias(esClient: EsOpsClientApi, alias: String, newIndexName: String, waitForElastic: Int = 1000)
  (implicit ec: ExecutionContext) {

  import esClient._

  def switchAlias: Future[Either[IndexError, StageSucceeded]] = trySwitching
    .recover { case NonFatal(ex) =>
        Left(IndexError("Could not switch alias with: " + ex.toString + "\n" + ex.getStackTrace.mkString("\n")))
    }

  private def trySwitching: Future[Either[IndexError, StageSucceeded]] = for {
    _ <- Future(Thread.sleep(waitForElastic))
    oldSize <- latestIndexWithAliasSize(alias)
    newSize <- sizeFor(newIndexName)
    optSwitchRes <- oldSize.traverse(size => switchAliasBetweenIndices(newSize / size.toDouble))
  ...
  private def switchAliasBetweenIndices(percentage: Double): Future[Either[IndexError, StageSucceeded]] =
