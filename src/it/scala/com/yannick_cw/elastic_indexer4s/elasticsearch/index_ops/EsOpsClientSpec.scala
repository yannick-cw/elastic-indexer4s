...
class EsOpsClientSpec extends AsyncWordSpec with ItSpec {
  ...
  "The EsOpsClient" should {
    ...
    "be able to add an alias to an index" in {
      ...
      } yield indicesInfo.find(_.index == indices.head).get.aliases shouldBe List("alias")
    }
    ...
    "be able to retrieve indexInfo" in {
      for {
        ...
        forAll(indexInfos)(info => info.aliases should contain theSameElementsAs List("alias", s"alias_${info.index}"))
      }
    }
  }
}
